...
let coords = [];
data.forEach(contact => {
  coords.push(contact.address.geo.lat, contact.address.geo.lng);
});
// console.log(coords);
// create an array of strings that combine the company's catchPhrase and bs
const phrases = [];
data.forEach(contact => {
  phrases.push(`${contact.company.catchPhrase}, ${contact.company.bs}`);
});
console.log(phrases);
// query the HTML for the class class-week, then use forEach to change the text content
document.querySelectorAll(".class-week").forEach((element, index) => {
  element.textContent = `${element.textContent} - index: ${index}`;
});
